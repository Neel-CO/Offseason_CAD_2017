...
path=%path%;c:\Program Files\PTC\Creo 2.0\Parametric\bin
...
echo Renumbering.......... %1
for /F "tokens=1,2,3 delims=. " %%I in ('dir /b *.asm.*
*.prt.* *.drw.* *.frm.* *.gph.* *.win.* *.tbl.* *.sym.*') do (
if %%K GTR 0 Call :ReNameFile %%I %%J %%K
)
time /t
goto :Cleaning

:ReNameFile
if exist %1.%2.%3 ren %1.%2.%3 %1.%2
goto :complete
